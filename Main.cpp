...
Program Description:
This program is the fourth programming assignment for CSE2383; implement an AVL tree.
*/
	...
	while (ProgramLoop)
	{
		try
		{
			...
			else if (input == "9")
			{
				...
				while (!ValidName)
				{
					cout << "Enter the filename to load the contents of the AVL from\n >> ";
				...
				ifstream getTree(filename + ".txt");

				//Line of code to read a file's contents to a string from: 
				https://stackoverflow.com/questions/2602013/read-whole-ascii-file-into-c-stdstring

				string filePost((std::istreambuf_iterator<char>(t)), std::istreambuf_iterator<char>());

				cout << "Postorder we will build from: " << filePost << endl;
			}
			...
			{
				//As a debug feature for the AVL, this will pause on this line and calculate the balance factor of every node
